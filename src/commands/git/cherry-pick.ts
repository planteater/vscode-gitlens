...
import { GitBranch, GitReference, GitRevision, Repository } from '../../git/git';
import {
	appendReposToTitle,
	PartialStepState,
	pickBranchOrTagStep,
		...
		while (this.canStepsContinue(state)) {
			...
			if (state.counter < 3 && context.selectedBranchOrTag != null) {
				const result: StepResult<GitReference[]> = yield* pickCommitsStep(
					...
					context,
					{
						log: await Container.git.getLog(state.repo.path, {
							ref: GitRevision.createRange(context.destination.ref, context.selectedBranchOrTag.ref),
							merges: false,
						}),
