...
// function executeCommand<T>(command: Commands, args: T) {
// 	return commands.executeCommand(command, args);
// }

function executeEditorCommand<T>(command: Commands, uri: Uri | undefined, args: T) {
	return commands.executeCommand(command, uri, args);
}

export function executeGitCommand(args: GitCommandsCommandArgs) {
...
export namespace GitActions {
	...
	// @log()
	// static explore(node: ViewRefNode) {
	...
	// 			repoPath = node.commit.repoPath;
	// 			files = node.commit.files;
	// 			ref1 = node.commit.previousSha !== undefined ? node.commit.previousSha : GitRevision.deletedOrMissing;
	...
	export namespace Commit {
		...
		export async function openChanges(
			...
			const refs =
				commitOrRefs instanceof GitLogCommit
					? {
							repoPath: commitOrRefs.repoPath,
							ref1:
								commitOrRefs.previousSha !== undefined
									? commitOrRefs.previousSha
			...
			void (await commands.executeCommand(Commands.DiffWith, {
		...
		export async function openAllChanges(
			...
			let files;
			let refs;
			if (commitOrFiles instanceof GitLogCommit) {
				files = commitOrFiles.files;
				refs = {
					repoPath: commitOrFiles.repoPath,
					ref1:
						commitOrFiles.previousSha !== undefined
							? commitOrFiles.previousSha
							: GitRevision.deletedOrMissing,
		...
		export async function openChangesWithWorking(
			...
			let ref;
			if (commitOrRef instanceof GitLogCommit) {
				ref = {
		...
		export async function openFile(file: GitFile, commit: GitLogCommit, options?: TextDocumentShowOptions) {
			options = { preserveFocus: true, preview: false, ...options };

			await executeEditorCommand<OpenWorkingFileCommandArgs>(Commands.OpenWorkingFile, undefined, {
				uri: GitUri.fromFile(file, commit.repoPath, commit.sha),
				showOptions: options,
			});
		}
		...
		export async function openRevision(commit: GitLogCommit, file: GitFile, options?: TextDocumentShowOptions) {
			options = { preserveFocus: true, preview: false, ...options };

			void (await findOrOpenEditor(
				GitUri.toRevisionUri(file.status === 'D' ? commit.previousFileSha : commit.sha, file, commit.repoPath),
				options,
			));
		}

		export async function openRevisions(commit: GitLogCommit, options?: TextDocumentShowOptions) {
			if (commit.files.length > 20) {
				const result = await window.showWarningMessage(
			...
			for (const file of commit.files) {
				void (await openRevision(commit, file, options));
			}
		}

		export async function restoreFile(commit: GitLogCommit, file: string | GitFile) {
			const fileCommit = commit.toFileCommit(file);
			if (fileCommit == null) return undefined;

			return Container.git.checkout(commit.repoPath, commit.ref, { fileName: fileCommit.fileName });
		}
	}

	export namespace Contributor {
		export async function addAuthors(repo?: string | Repository, contributor?: GitContributor) {
			return executeGitCommand({
				command: 'co-authors',
				state: { repo: repo, contributors: contributor != null ? [contributor] : undefined },
			});
		}
	...
	export namespace Remote {
		...
		// 	@log()
		// 	static async pruneRemote(node: RemoteNode) {
	...
	export namespace Stash {
		...
		export async function push(repo?: string | Repository) {
			return executeGitCommand({
				command: 'stash',
				state: { subcommand: 'push', repo: repo },
			});
		}
	}
}
